...
const ConversationSidebar = () => {
    ...
    return (
        ...
        >
            <div className={`h-screen fixed ${isOpenSideBar ? 'w-56' : 'w-16'} bg-white border-gray-200 border-r dark:bg-gray-800 dark:border-gray-700 text-black dark:text-white overflow-y-scroll custom-scrollbar`}>

            <div className=" flex items-center gap-3 flex-col">
                {/* create conversation components*/}
                <div className="flex w-full items-center cursor-pointer gap-2 p-2 sticky bg-gray-200 dark:bg-gray-900 top-0" onClick={() => setIsOpenCreateNew(true)}>
                {/* create icon */}
                ...
                <Link to={"/msg/" + conversation.id} className={`flex w-full items-center gap-2 p-2 cursor-pointer duration-300 ease-in-out hover:bg-gray-200 hover:dark:bg-gray-900 ${conversation.id === id ? 'bg-gray-200 dark:bg-gray-900' : ''}`} key={conversation.id}>
                    <img
                    ...
                    {isOpenSideBar && <div>
                        ...
                        <p className="text-gray-500">{conversation.lastMessage?.content ? conversation.lastMessage.content : "Nhóm đã được tạo"}</p>
                    </div>}
                    </div>
                </Link>
                ))}
